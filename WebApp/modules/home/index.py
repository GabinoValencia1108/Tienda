...
def registro():
    ...
    paquetes=StockTienda.query.filter(StockTienda.categoria=="paquetes").all()
...
def proveedores():
    ...
    if form.validate_on_submit():
        Producto = Proveedores_Tienda(request.form['id_proveedores'],request.form['nombre_proveedor'],request.form['numero_pedido'])
        db.session.add(Producto)
        db.session.commit()
